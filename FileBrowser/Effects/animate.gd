...
static func drop_window(old_window: Window):
	...
	var tilt_excess = 128
	fake_window.size = Vector2i(old_window.size.x + tilt_excess, DisplayServer.screen_get_size().y - old_window.position.y + tilt_excess)
	fake_window.position = Vector2i(old_window.position.x - tilt_excess/2, old_window.position.y )
	...
	fake_window.set_flag(Window.FLAG_MOUSE_PASSTHROUGH, true)
	...
	image.position = Vector2(tilt_excess/2, 0)
